  ...
  return (
    ...
    <div id="product-display-image" className="leading-none absolute bottom-[15%] flex flex-col items-center justify-center gap-[10px]">
      <motion.div className="w-max h-max" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: animationTokens.duration1, delay: animationTokens.delay1 }}>
        <Image width={50} height={50} className="object-cover" src="/assets/images/parental-guidance.png" alt="integrity-cover" priority />
      </motion.div>
      <div className="flex flex-row gap-2">
      {albumTextParts.map((part, index) => (
        <React.Fragment key={index}>
          <motion.h1 style={styles.subText} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: animationTokens.duration2, delay: index * animationTokens.delay2 / 3 }}>
      ...
      <motion.button className="cursor-pointer" style={styles.button} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: animationTokens.duration4, delay: animationTokens.delay4 }}>
